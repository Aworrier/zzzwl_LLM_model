...
class Transformer(PreTrainedModel):
    ...
    def __init__(self, args: ModelConfig = None):
        ...
        # 将隐藏状态映射回词表空间，得到每个token的logits。
        # 可以看到输出是一个线性层，输入是每个token的隐藏状态，输出是每个token的logits

        # 将词嵌入层的权重与输出层的权重共享，提升泛化能力
        self.tok_embeddings.weight = self.output.weight 

        # 预计算相对位置嵌入的频率：位置编码（RoPE）
    ...
    # 支持自回归生成，支持温度采样、top-k采样和停止token。
