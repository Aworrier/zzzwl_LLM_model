...
class TextGenerator:
    def __init__(self, 
                 checkpoint='/mnt/data/zzzwl/happy-llm/zwl_mymodel/zwl_SFT/zzzwl_model_sft/sft_dim1024_layers18_vocab_size6144.pth',  # 模型检查点路径
                 tokenizer_model_path='/mnt/data/zzzwl/happy-llm/zwl_mymodel/zwl_SFT/tokenizer_k',  # 分词器模型路径
                #  seed=36,  # 随机种子，确保可重复性
                 seed=42,  # 可以将其删除掉。原先的值为42
...
if __name__ == "__main__":
    ...
    generator = TextGenerator(checkpoint='/mnt/data/zzzwl/happy-llm/docs/chapter5/code/model_base/pretrain_1024_18_6144.pth')  # 初始化生成器
    ...
    generator = TextGenerator(checkpoint='/mnt/data/zzzwl/happy-llm/zwl_mymodel/zwl_SFT/zzzwl_model_sft/sft_dim1024_layers18_vocab_size6144.pth')  # 初始化生成器
    for i in range(len(sft_prompt_datas)):
        samples = generator.sft_sample(start=sft_prompt_datas[i], num_samples=5, max_new_tokens=128, temperature=0.6)
